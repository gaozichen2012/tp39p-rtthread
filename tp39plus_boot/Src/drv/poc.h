...
/*
#define AUDIO_GPIO_POC_RST_PORT   GPIOA
#define AUDIO_GPIO_POC_RST_PIN    GPIO_Pin_9		///---ON/OFF 引脚
*/

#define AUDIO_GPIO_POC_RST_PORT   GPIOB
#define AUDIO_GPIO_POC_RST_PIN    GPIO_Pin_2		///---ON/OFF 引脚
...
typedef struct
{
    uint32_t boot_flag; //此标志为了让boot程序识别，进入update模式，boot_flag=0x88888888则进入update
    uint8_t soft_ver[32];    //版本
	uint8_t sim_card_select;//0:SIM1 1:SIM2
} poc_flash_boot_t;

void poc_init(void);
void poc_send_cmd(char*  poc_cmd);  //发送AT指令  poc_cmd_t
void poc_task(void);
