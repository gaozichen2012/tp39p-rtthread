...
static void poc_gpio_init()
{
	...
	RCC_AHBPeriphClockCmd( RCC_AHBPeriph_GPIOA, ENABLE);
	GPIO_InitStructure.GPIO_Pin =   DEVICE_GPIO_POWER_PIN ;	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
	GPIO_Init(DEVICE_GPIO_POWER_PORT, &GPIO_InitStructure);
	POC_POWER_ON();
}
...
#if 1
void poc_send_cmd(char* poc_cmd)
{
	uart2_send_string((const uint8_t*)poc_cmd, strlen(poc_cmd));			
}
#else
...
//		device_config_read_flash();
		...
		if(memcmp(ptr+14 ,boot_info.soft_ver,12) == 0)//对比前12位"TYT_TP39PV2_",相同则进入下一步
		{
			if(memcmp(ptr+14,boot_info.soft_ver,16) == 0)//版本号一样则退出升级
			{
			...
			else
			{
				...
				memset(&poc_device.server_soft_ver[16],0,32-16);
				memcpy(poc_device.server_soft_id,ptr+9,4);	

				poc_device.down_pack_cnt = 0x0;
				poc_device.down_status = 0x00;
			}
		}
	}
...
void poc_task(void)
{
	...
	switch(poc_device.status)
	{
		...
		case POC_DEVICE_STATUS_CHECKED_OK:
		{
			...
			{
				resend_timer = SysTick_Cnt;
				poc_send_cmd(POC_CMD_POC_OPENING);
			}					
		}break;
		...
		case POC_DEVICE_STATUS_LAND_SUCCESSED:
		{
			/*
			#if(DEF_DEBUG)
			if(poc_device.server_soft_ver[0] != 0)
			{
				printf("server_soft_ver=%s\r\n",poc_device.server_soft_ver);
			}
			#endif
			*/
			//if(poc_device.server_soft_ver[0] == 0) //检测版本号
			//{
				if(SysTick_Cnt - resend_timer > 1000)
				{
					resend_timer = SysTick_Cnt;
					uart2_send_string((const uint8_t*)"AT+POC=420000\r\n", strlen("AT+POC=420000\r\n"));	
				}				
			//}			
		}break;
		...
		case POC_DEVICE_VER_DWONLOAD://下载固件到模块
		{
			...
			{
				if((abs(SysTick_Cnt - resend_timer) > 5000 || send_times ==0)&&(poc_device.down_status == 0))
				{	
...
#if(DEF_DEBUG)
void printf_Device_status(void)
{
const char *statuschar[]=
{
	...
	static poc_device_status_t Prev_status=POC_DEVICE_VER_SAME;
	
	if(Prev_status != poc_device.status)
	{
		printf("status=%s\r\n",statuschar[poc_device.status]);
		Prev_status = poc_device.status;
	}
}
